...
procedure cSDL3Eng.AddPoint;
begin
  SetLength(Points, Length(Points) + 1);
  Points[High(Points)].Init(Random * kRenderW, Random * kRenderH);
end;

procedure cSDL3Eng.RemovePoint;
begin
  if Length(Points) > 1 then
    SetLength(Points, Length(Points) - 1); // or High(Points)
end;
...
begin
  ...
  begin
    ...
    case DrawMode of
      dmDefault : Render.PolygonBorder(Points);
      dmSubPixel : Render.SPPolygonBorder(Points);
      // dmFullPixel : Render.FPPolygonBorder(PointsInt);
    end;
  end;
  ...
  begin
    ...
    case DrawMode of
      dmDefault : Render.PolygonFilled(Points);
      dmSubPixel : Render.SPPolygonFilled(Points);
      // dmFullPixel : Render.FPPolygonFilled(PointsInt);
    end;
  end;

  if (FillMode = fmBorFill) or (FillMode = fmAll) then
    case DrawMode of
      dmDefault : Render.Polygon(Points, BorderColor, FillColor);
      dmSubPixel : Render.SPPolygon(Points, BorderColor, FillColor);
      // dmFullPixel : Render.FPPolygon(PointsInt, BorderColor, FillColor);
    end;
...
begin
  ...
  Render.DebugText(0, 60, '[A] [Z] Change number of vertices');
  Render.DebugText(0, 70, '[P] Change points');
  Render.PopDrawColor;
