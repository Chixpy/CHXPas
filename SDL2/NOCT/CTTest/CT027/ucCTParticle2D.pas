...
type
  ...
  cCTParticle2DList = specialize TFPGObjectList<cCTParticle2D>;

implementation

{ cCTParticle2D }

procedure cCTParticle2D.applyForce(const force : TPoint3DF);
begin
  acceleration.add(force);
end;

procedure cCTParticle2D.update;
begin
...
function cCTParticle2D.explode : Boolean;
begin
...
function cCTParticle2D.isDead : Boolean;
begin
  Result := lifespan < 0.0;
end;

constructor cCTParticle2D.Create(const x, y : float;const aR, aG, AB : Byte);
begin
  // hu := h;
...
constructor cCTParticle2D.Create(const l : TPoint3DF;const aR, aG, AB : Byte);
begin
  // hu := h;
  acceleration := Point3DF(0, 0);
...
destructor cCTParticle2D.Destroy;
begin
  inherited Destroy;
end;
