...
uses
  ...
  ucCTCParticle2D, ucCTCFirework2D;
  ...
  begin
    ...
    if Random < 0.08 then
      fireworks.add(cCTCFirework2D.Create(RandomRange(0, WinW),
        WinH, RandomRange(0, 256), RandomRange(0, 256),
        RandomRange(0, 256), gravity));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        filledcircleRGBA(SDLWindow.PRenderer, Round(p.location.x),
          Round(p.location.y), r, p.r, p.g, p.b, Round(p.lifespan));
      end;
